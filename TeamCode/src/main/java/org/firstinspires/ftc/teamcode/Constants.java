package org.firstinspires.ftc.teamcode;

public class Constants {
    public static double grabberClose = 0.22;
    public static double grabberOpen = 0.55;

    public static double wristDown = 0.93;
    public static double wristUp = 0.28;

    public static double armSlowSpeed = 0.3;
    public static int armTop = 800;
    public static int armBack = 1300;

    public static int slideLow = 250;
    public static int slideMedium = 1070;

    public static double driveSpeedNormal = 0.75;
    public static double driveSpeedSlow = 0.4;
    public static double driveSpeedFast = 1.0;

    public static double driveTuningFR = 1.0;
    public static double driveTuningFL = 0.70;
    public static double driveTuningBR = 1.0;
    public static double driveTuningBL = 0.7;
}
